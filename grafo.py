...
class Grafo():
    ...
    def barrido_desigual_tipo(self):
        for i, value in enumerate(self.__elements):
            buscado_pais, buscado_tipo, buscado_nombre = value[0].pais, value[0].tipo, value[0].nombre

            for otro_value in self.__elements[i + 1:]:
                otro1, otro2, otro3 = otro_value[0].pais, otro_value[0].tipo, otro_value[0].nombre
                if buscado_pais == otro1 and buscado_tipo != otro2:
                    print(f'{buscado_pais} tiene {buscado_tipo}: {buscado_nombre}; y {otro2}: {otro3}')

    def barrido_mismo_tipo(self):
        for i, value in enumerate(self.__elements):
            buscado_pais, buscado_tipo, buscado_nombre = value[0].pais, value[0].tipo, value[0].nombre
        ...
        for value in self.__elements:
            for i in range(value[1].size()):
                ...
                if arista.peso > peso_mayor:
                    ...
                    conectado1, conectado2 = [value[0].nombre, arista.vertice]
